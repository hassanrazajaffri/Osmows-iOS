...
- (void)viewDidLoad {
    ...
    self.view.frame = CGRectMake(0, 0, screenWidth, (self.itemsArray.count*44));
    self.preferredContentSize = CGSizeMake(screenWidth, (self.itemsArray.count*44));
    self.popoverPresentationController.sourceView = self.sourceView;
    self.tableView = [[UITableView alloc] initWithFrame:self.view.bounds];
 //   self.tableView.frame=CGRectMake(5, self.tableView.frame.origin.y,  self.tableView.frame.size.width,  self.tableView.frame.size.height);
    ...
    self.popoverPresentationController.sourceRect = CGRectMake(0, 0, screenWidth, 30);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.itemsArray[indexPath.row] isKindOfClass:[NSDictionary class]]) {
       cell.textLabel.text = [self.itemsArray[indexPath.row] objectForKey:@"Location"];;
    }
    else{
