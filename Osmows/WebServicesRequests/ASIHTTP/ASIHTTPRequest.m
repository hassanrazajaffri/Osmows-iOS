...
@property (assign, nonatomic) BOOL readStreamIServiceScheduled;
...
- (void)startRequest
{
	...
	[self setReadStreamIServiceScheduled:NO];
	...
	// Schedule the stream
	if (![self readStreamIServiceScheduled] && (!throttleWakeUpTime || [throttleWakeUpTime timeIntervalSinceDate:[NSDate date]] < 0)) {
...
- (BOOL)shouldTimeOut
{
	...
	if ([self readStream] && [self readStreamIServiceScheduled] && [self lastActivityTime] && [self timeOutSeconds] > 0 && secondsSinceLastActivity > [self timeOutSeconds]) {
...
- (void)scheduleReadStream
{
	if ([self readStream] && ![self readStreamIServiceScheduled]) {
		...
		[self setReadStreamIServiceScheduled:YES];
	}
}
...
- (void)performThrottling
{
	...
	if ([ASIHTTPRequest isBandwidthThrottled]) {
		...
		if (throttleWakeUpTime) {
			if ([throttleWakeUpTime timeIntervalSinceDate:[NSDate date]] > 0) {
				if ([self readStreamIServiceScheduled]) {
					[self unscheduleReadStream];
					#if DEBUG_THROTTLING
	...
	} else if (![self readStreamIServiceScheduled]) {
		[self scheduleReadStream];			
	}
}
...
@synthesize readStreamIServiceScheduled;
