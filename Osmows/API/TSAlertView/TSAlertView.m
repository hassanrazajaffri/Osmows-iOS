...
const CGFloat kTSAlertView_LeftMargin	= 15.0;
const CGFloat kTSAlertView_TopMargin	= 5.0;
...
- (void) TSAlertView_commonInit
{
	...
	self.width = 217; // set to default
	self.maxHeight = 143; // set to default
...
- (void) pulse
{
	...
	// pulse animation thanks to:  http://delackner.com/blog/2009/12/mimicking-TSAlertViews-animated-transition/
...
- (CGSize) recalcSizeAndLayout: (BOOL) layout
{
	...
	if ( layout )
	{
		...
		if ( self.message != nil )
		{
			...
			{
				y += 1;
				self.messageLabel.frame = CGRectMake( kTSAlertView_LeftMargin, y+15, messageViewSize.width, messageViewSize.height );
		...
		if ( self.style == TSAlertViewStyleInput )
		{
			self.inputTextField.frame = CGRectMake( kTSAlertView_LeftMargin, y-15, inputTextFieldSize.width, inputTextFieldSize.height );
		...
		if ( stacked )
		{
			...
			{
				b.frame = CGRectMake( kTSAlertView_LeftMargin, y+18, buttonWidth, buttonHeight );
				[self addSubview: b];
